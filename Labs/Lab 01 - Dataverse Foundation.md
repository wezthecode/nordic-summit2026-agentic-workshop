...
**Authoritative spec:** [`dataverse-schema.md`](dataverse-schema.md) — read this first
...
- The two tables (`cre_project`, `cre_timeentry`) with the columns and constraints from the [schema spec](dataverse-schema.md)
- 3 seeded projects from [`projects.csv`](../WorkshopStarterSolution/projects.csv)
...
Columns from the [schema spec](dataverse-schema.md#table-1--cre_project). Mark as **organisation-owned**. Set primary name column to `cre_name`.

### 3. Seed `cre_project`

Import [`projects.csv`](../WorkshopStarterSolution/projects.csv) via the maker portal table import. 3 rows.

### 4. Create `cre_timeentry`

Columns from the [schema spec](dataverse-schema.md#table-2--cre_timeentry). **User-owned.** Primary name column is `cre_name` with a business rule to auto-populate when blank (see spec).
...
All four rules in the [Validation table](dataverse-schema.md#validation-no-overlap-no-duplicate-no-future-dated-entries):
...
Privileges per the [security-role table](dataverse-schema.md#security-role) in the spec. Assign it to **your own user** in this environment.

### 7. Create the model-driven app

`Weekly Timesheet` per the [MDA section](dataverse-schema.md#model-driven-app). Enable Copilot on the app — we'll show it side-by-side with the standalone agents in lab 02.
...
Drop both into `../Completed_Labs/dataverse/`. These are your recovery artefact for the rest of the workshop.

## You will leave this lab with

- Working solution in your dev environment
- Exported solution zips in `Completed_Labs/dataverse/`
...
| Project lookup is empty when seeding from CSV | Wrong column header names | Match the exact headers in [`projects.csv`](../WorkshopStarterSolution/projects.csv) |
...
Move to [Lab 02 - Copilot Studio Build](Lab%2002%20-%20Copilot%20Studio%20Build.md).
